...
model = tf.keras.models.load_model('osero_model_2.h5')
...
class State:
    ...
    def drop(self):
      if self.pieces.shape == tf.constant([]).shape:
          ...
          end_kihu_put = end_kihu[:, :, BOARD_SIZE*2+1:]
          ...
          if jud_rabel.shape == tf.constant([]).shape:
              ...
              end_kihu = tf.concat([end_kihu, end_kihu_put], 2)
              end_kihu = tf.reshape(end_kihu,(len(end_kihu)*len(end_kihu[0]),BOARD_SIZE*3+1))#2(1)次元化
    ...
    def keep(self, next_pieces):
      if self.pieces.shape == tf.constant([]).shape:
          ...
          now_full = tf.math.logical_or(self.pieces[:, :BOARD_SIZE], self.pieces[:, BOARD_SIZE:BOARD_SIZE*2])
          now_empty = tf.math.logical_not(now_full)
          next_full = tf.math.logical_or(next_pieces[:, :BOARD_SIZE], next_pieces[:, BOARD_SIZE:BOARD_SIZE*2])
          put_point = tf.math.logical_and(now_empty, next_full)
          pieces_r_put = tf.concat([pieces_r, put_point], 1)
          if type(self.kihu) == type(None):
            self.kihu = tf.reshape(pieces_r_put, (len(pieces_r), 1, BOARD_SIZE*3+1))
          else:
            pieces_r_put = tf.reshape(pieces_r_put, (len(pieces_r),1,BOARD_SIZE*3+1))
            self.kihu = tf.concat([self.kihu, pieces_r_put], 1)
    ...
    def tri_predict(self):
      ...
      next = tf.reshape(next, [next.shape[0], BOARD_SIZE*2])
      self.keep(next)
      self.drop()
      return next
    ...
    def predicter(self):
      ...
      for i,j in enumerate(legal_board_num):
        if j == 0:
          next = next.write(i, tf.concat([self.pieces[i,:,:,1], self.pieces[i,:,:,0]], 3))
        else:
